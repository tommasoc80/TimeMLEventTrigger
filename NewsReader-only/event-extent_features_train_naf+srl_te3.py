...
##############
    ...
    for k, v in token_dependency.items():
        ...
        f = naff.split("/")[-1]
        final_val_extent = (f,) + (str(new_sent_id),) + v[2:]

        final_extent[int(k_ord)] = final_val_extent
        ...
        if int(k) == 0 and int(v[2]) == 1:
            ...
            output.writelines(
                v[0] + "\t" + str(k) + "\t" + '\t'.join(v[1:6]) + "\t" + "\t".join(v[8:11]) + "\t" + "\t".join(
                    ...
                    v[19:-1]) + "\t" + v[-1].replace('B-OCCURRENCE', 'B-EVENT').replace('B-I_ACTION',
                                                                                        'B-EVENT').replace('B-STATE',
                                                                                                           'B-EVENT').replace(
                    'B-ASPECTUAL', 'B-EVENT').replace('B-I_STATE', 'B-EVENT').replace('B-REPORTING', 'B-EVENT').replace(
                    'B-PERCEPTION', 'B-EVENT') + "\n")
            output.close()
...
def main(argv=None):
    ...
    if len(argv) < 5:
        print('Usage: python3 features_test.py naf_folder train_dir predicateMatrix wordnet30_noun_supersenses outdir')
    else:
