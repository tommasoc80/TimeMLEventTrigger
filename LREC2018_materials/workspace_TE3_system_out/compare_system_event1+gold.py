...
"""
compare 2 system output for event detection and classification against the gold
"""


def get_data(sys1_out, sys2_out, gold_data, outdir):
...
#        print k, v
        ...
        if k in dict_event_extent_sys1 and k in dict_event_extent_sys2:
            text_file = io.open(outdir + "common_gold_extent", "a", encoding="utf-8")
            text_file.write(unicode(k + "\t" +  token_text[k]) + "\n")
            text_file.close()

        if not k in dict_event_extent_sys1 and not k in dict_event_extent_sys2:
    ...
    for k, v in dict_event_class_gold.iteritems():
        ...
        if k in dict_event_class_sys1 and k in dict_event_class_sys2:
            if v == dict_event_class_sys1[k] and v == dict_event_class_sys2[k]:
        ...
        if k in dict_event_class_sys1 and k in dict_event_class_sys2:
            if v != dict_event_class_sys1[k] and v != dict_event_class_sys2[k]:
                ...
                text_file.write(unicode(k + "\t" + token_text[k] + "\t" + v + "\t" + dict_event_class_sys1[k] + "\t" + dict_event_class_sys2[k]) + "\n")
                text_file.close()


def contingent_matrix_event(sys1dir, sys2dir, gold_dir, outdir):
...
#            get_data(sys1dir + f, sys2f, outfile)
            get_data(sys1dir + f, sys2f, goldf, outdir)
        else:
...
def main(argv=None):
    ...
    if len(argv) < 5:
        print 'Usage: python compare_system_event1.py sys1 sys2 gold outdir'
    else:
