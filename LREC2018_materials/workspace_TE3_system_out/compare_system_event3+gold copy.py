...
def get_data(gold_data, sys1_out, sys2_out, sys3_out, sys4_out, sys5_out, sys6_out, outdir):
    ...
    for elem in root_gold.findall('Markables/EVENT'):
        ...
        for tokens in elem.findall('token_anchor'):
            ...
            dict_event_extent_gold[doc_gold + "\t" + token_id] = event_id + "\t" + event_pos
            dict_event_class_gold[doc_gold + "\t" + token_id] = event_class + "\t" + event_pos
   ...
   """
    ...
    for k, v in dict_event_extent_gold.iteritems():
        ...
        if k in dict_event_extent_sys1:
            event_extent[k + "\t" + POS].append(int(1))
        else:
            event_extent[k + "\t" + POS].append(int(0))
...
#            event_extent[k].append(int(0))
    ...
    for k, v in event_extent.iteritems():
        k_splitted = k.split("\t")
        match_extent = k_splitted[0] + "\t" + k_splitted[1]
        if match_extent in token_text:
            event_extent_tokens_gold_systems[k + "\t" + token_text[match_extent]] = sum(v)
    ...
    for k, v in dict_event_class_gold.iteritems():
        ...
        if k in dict_event_class_sys1:
            if dict_event_class_sys1[k] == v_splitted[0]:
                event_class[k + "\t" + v].append(int(1))
            else:
                event_class[k + "\t" + v].append(int(0))

        if k in dict_event_class_sys2:
            if dict_event_class_sys2[k] == v_splitted[0]:
                event_class[k + "\t" + v].append(int(1))
            else:
                event_class[k + "\t" + v].append(int(0))

        if k in dict_event_class_sys3:
            if dict_event_class_sys3[k] == v_splitted[0]:
                event_class[k + "\t" + v].append(int(1))
            else:
                event_class[k + "\t" + v].append(int(0))

        if k in dict_event_class_sys4:
            if dict_event_class_sys4[k] == v_splitted[0]:
                event_class[k + "\t" + v].append(int(1))
            else:
                event_class[k + "\t" + v].append(int(0))

        if k in dict_event_class_sys5:
            if dict_event_class_sys5[k] == v_splitted[0]:
                event_class[k + "\t" + v].append(int(1))
            else:
                event_class[k + "\t" + v].append(int(0))

        if k in dict_event_class_sys6:
            if dict_event_class_sys6[k] == v_splitted[0]:
                event_class[k + "\t" + v].append(int(1))
            else:
                event_class[k + "\t" + v].append(int(0))
...
def main(argv=None):
    ...
    if len(argv) < 9:
        print 'Usage: python compare_system_event2+Gold-v2.py gold sys1 sys2 sys3 sys4 sys5 sys6 outdir'
    else:
