...
def process_predicted(out_f):

    predicted_data = {}

    fileObject = open(out_f)
    for line in fileObject:
        line_stripped = line.strip()
        line_splitted = line_stripped.split("\t")
        ...
        if len(line_splitted) > 1:
            ...
            predicted_event = line_splitted[-1]

            predicted_data[int(token_id)] = predicted_event
...
def process_features_stanford(stanfordf, naf_f,  out_f, predicateMatrix, wn_supersenses, outfile):
...
#################
    ...
    predicted = process_predicted(out_f)
        ...
        for k1, v1 in sem_roles_naf.items():
            ...
            for i in v1:
                if i == v[0]:
                    ...
                    if new_val in predicted:
                      
                        if predicted[new_val] != "O":
    ...
    for k, v in final_class.items():
        if k in predicted:
            if predicted[k] != "O":
                ...
                new_val = v + ("O", "O", "O", "O", "O",) + (predicted[k],) + ("O",)
                final[k] = new_val
...
def stanford_features(stanfordir, nafdir, predicteddir, predicateMatrix, wn_supersenses, outdir):

    for f in os.listdir(stanfordir):
        if f.endswith(".xml"):

            outfile = outdir + f + ".features.class-test"

            out_f = predicteddir + f + ".features.extent-test.crf_out"
            process_predicted(out_f)

            naf_f = nafdir + f.split(".txt.xml")[0] + ".features.naf"
            process_feature_naf (naf_f)

            process_features_stanford(stanfordir + f, naf_f, out_f, predicateMatrix, wn_supersenses, outfile)
...
def main(argv=None):
    ...
    if len(argv) < 7:
        print('Usage: python3 event-class_features_test_stanford+naf_te3.py [stanford_folder] [naf_folder] predicted_extent_dir predicateMatrix wordnet30_noun_supersenses outdir')
    else:
