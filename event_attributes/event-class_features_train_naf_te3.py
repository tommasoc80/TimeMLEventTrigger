...
##############
    ...
    for k, v in final_class.items():
        if k in training:
            if training[k][-1] != "O":
                new_val = (v[0],) + v[2:] + training[k][0:-1] + ("B-EVENT",) + (training[k][-1],)
                final[k] = new_val
...
def naf_features(nafdir, traindir, predicateMatrix, wn_supersenses, outdir):
    ...
    for f in os.listdir(nafdir):
        if f.endswith(".naf"):
            ...
            outfile = outdir + f + ".features.class-train"
...
def main(argv=None):
    ...
    if len(argv) < 6:
        print('Usage: python3 event-class_features_train_naf_te3.py [naf_folder] [train_dir] predicateMatrix wordnet30_noun_supersenses outdir')
    else:
