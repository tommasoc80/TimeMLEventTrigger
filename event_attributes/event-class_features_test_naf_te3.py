...
def process_features_naf(naff, out_f, predicateMatrix, wn_supersenses, outfile):
...
def naf_features(nafdir, predicteddir, predicateMatrix, wn_supersenses, outdir):
    ...
    for f in os.listdir(nafdir):
        if f.endswith(".naf"):
            ...
            if os.path.isfile(out_f):
                ...
                process_features_naf(nafdir + f, out_f, predicateMatrix, wn_supersenses, outfile)
...
def main(argv=None):
    ...
    if len(argv) < 6:
        print('Usage: python3 event-class_features_test_naf_te3.py [naf_folder] [predicted_event_extent_dir] predicateMatrix wordnet30_noun_supersenses outdir')
    else:
