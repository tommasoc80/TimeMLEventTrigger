...
def stanford_features(stanfordir, nafdir, predicteddir, predicateMatrix, wn_supersenses, outdir):
    ...
    for f in os.listdir(stanfordir):
        if f.endswith(".xml"):
            ...
            outfile = outdir + f + ".features.tense-test"
...
def main(argv=None):
    ...
    if len(argv) < 7:
        print('Usage: python3 event-tense_features_test_stanford+naf_te3.py [stanford_folder] [naf_folder] predicted_extent_dir predicateMatrix wordnet30_noun_supersenses outdir')
    else:
