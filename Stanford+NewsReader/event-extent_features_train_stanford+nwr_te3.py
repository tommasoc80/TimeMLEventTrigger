...
#            event_class = line_splitted[4].replace('B-OCCURRENCE', 'B-EVENT').replace('B-I_ACTION', 'B-EVENT').replace('B-STATE', 'B-EVENT').replace('B-ASPECTUAL', 'B-EVENT').replace('B-I_STATE', 'B-EVENT').replace('B-REPORTING', 'B-EVENT').replace('B-PERCEPTION', 'B-EVENT')
            ...
            if token_id_align == -99:
                train_data[0] = (tense, aspect, modality, polarity, event_id, event_class)
            else:
                train_data[token_id_align] = (tense, aspect, modality, polarity, event_id, event_class)
...
###################
    ...
    for k, v in token_dependency.items():
        ...
        final_val_extent = (f,) + (str(new_sent_id),) + (k_splitted[1],) + v[1:]

        final_extent[new_unique_token_id] = final_val_extent


    for k, v in final_extent.items():
        if k in training:
            if v[1] == "-1":
                new_val = (v[0],) + ("-99",) + v[2:] + training[k]
                final_extent[k] = new_val
            else:
                new_val = v + training[k]
                final_extent[k] = new_val
    ...
    for k, v in final_extent.items():
        if int(k) == 0 and int(v[2]) == 1:
            ...
            output.writelines(v[0] + "\t" + str(k) + "\t" + '\t'.join(v[1:6]) + "\t" + "\t".join(v[8:11]) + "\t" + "\t".join(v[14:19]) + "\t" + v[6] + "\t" + v[7] + "\t" + "\t".join(v[11:14]) + "\t" + "\t".join(v[19:-1]) + "\t" + v[-1].replace('B-OCCURRENCE', 'B-EVENT').replace('B-I_ACTION', 'B-EVENT').replace('B-STATE', 'B-EVENT').replace('B-ASPECTUAL', 'B-EVENT').replace('B-I_STATE', 'B-EVENT').replace('B-REPORTING', 'B-EVENT').replace('B-PERCEPTION', 'B-EVENT') +  "\n")
            output.close()

        elif int(k) != 0 and int(v[2]) == 1:
            output = open(outfile, 'a')
            output.writelines("\n" + v[0] + "\t" + str(k) + "\t" + '\t'.join(v[1:6]) + "\t" + "\t".join(v[8:11]) + "\t" + "\t".join(v[14:19]) + "\t" + v[6] + "\t" + v[7] + "\t" + "\t".join(v[11:14]) + "\t" + "\t".join(v[19:-1]) + "\t" + v[-1].replace('B-OCCURRENCE', 'B-EVENT').replace('B-I_ACTION', 'B-EVENT').replace('B-STATE', 'B-EVENT').replace('B-ASPECTUAL', 'B-EVENT').replace('B-I_STATE', 'B-EVENT').replace('B-REPORTING', 'B-EVENT').replace('B-PERCEPTION', 'B-EVENT') +  "\n")
            output.close()
...
def stanford_features(stanfordir, nafdir, traindir, predicateMatrix, wn_supersenses, outdir):
    ...
    for f in os.listdir(stanfordir):
        if f.endswith(".xml"):
            ...
            outfile = outdir + f + ".features.extent-train"
...
def main(argv=None):
    ...
    if len(argv) < 7:
        print('Usage: python3 features_test.py stanford_folder naf_folder train_dir predicateMatrix wordnet30_noun_supersenses outdir')
    else:
